...
public class CodeBPDateExtractionService implements Runnable {
	...
	public CodeBPDateExtractionService(BPBusinessControl businessControl) {
	...
	public void run() {
		...
		while (true) {
			...
			System.out.println("CodeBPDateExtractionService 。。。 。。。");
			...
			msgAgreement
					.setMsgBarcodePrinterEnum(MsgBarCodePrinterEnum.C2S_ORDERCODEINFO);
			...
			System.out.println("CodeBPDateExtractionService jsonStr 长度：");
			System.out.println(jsonStr.length());
			
			try {
				System.out.println("休眠5 秒钟");
